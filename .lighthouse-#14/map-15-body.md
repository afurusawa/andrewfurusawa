...
- [Research Next.js patterns for an isolated /90s experiment route](https://github.com/afurusawa/andrewfurusawa/issues/19) — Nested app/90s is fine; root chrome/globals leak unless slimmed root or multi-root groups; noindex + omit sitemap for secret URL.
- [Research accessible techniques for 90s kitsch UI patterns](https://github.com/afurusawa/andrewfurusawa/issues/20) — Kitsch look is AA-compatible when motion is pauseable/reduced-motion-aware, neon text hits contrast floors, GIF chrome is decorative (null alt), fake counters are non-interactive, frames-feel uses landmarks+reflow (not frameset), and neon controls keep dual-tone focus rings.
- [Choose the /90s information architecture and nav labels](https://github.com/afurusawa/andrewfurusawa/issues/16) — Single-page `/90s` only; nav About · Skills · Contact; no theater nav targets (chrome-only kitsch).
- [Define the must-have Neon Cyber Basement chrome inventory for v1](https://github.com/afurusawa/andrewfurusawa/issues/17) — Lean must shell (palette, type, frames nav, cards, counter, dividers, footer, focus) + optional pack (UC art, 2–4 badges, one texture); defer web-ring, badge walls, blink on substance.
- [Define core-trio content model and data source for /90s](https://github.com/afurusawa/andrewfurusawa/issues/18) — Share links+skills; homepage-parity About; skills as full period table (no filter); contact mailto+GitHub+LinkedIn, no form.
- [Set accessibility and motion policy for the /90s experiment](https://github.com/afurusawa/andrewfurusawa/issues/21) — WCAG 2.2 AA whole page; gentle motion only without reduced-motion; theater non-interactive for v1; functional theater deferred; no intentional AA trade-offs.
...
- Real period backends (guestbook storage/moderation, real hit counters, webring membership services) — including functional theater deferred to a later effort after this map.
